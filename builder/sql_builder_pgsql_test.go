...
func TestItCanGenerateInsertCommandPgsql(t *testing.T) {
	builder := builder.NewPgsqlBuilder(context.Background(), nil, "testing_table")
	...
	assert.Contains(t, builder.ToQueryString(), "$1")
	assert.Contains(t, builder.ToQueryString(), "$2")
	assert.Contains(t, builder.ToQueryString(), "$3")
...
func TestItCanGenerateUpdateCommandPgsql(t *testing.T) {
	builder := builder.NewPgsqlBuilder(context.Background(), nil, "testing_update")
...
func TestItCanGenerateWhereCommandPgsql(t *testing.T) {
	builder := builder.NewPgsqlBuilder(context.Background(), nil, "testing_update")
	t.Run("Testing it can generate 'where' conditional queries", func(t *testing.T) {
		...
		assert.Contains(t, builder.ToQueryString(), "WHERE conditional=$1")
	...
	t.Run("Testing it can generate 'and' conditional query when where invoked more than 1", func(t *testing.T) {
		...
		assert.Contains(t, builder.ToQueryString(), "AND another_conditional=$2")
	...
	t.Run("Testing it can generate 'or' conditional query", func(t *testing.T) {
		...
		assert.Contains(t, builder.ToQueryString(), "OR or_pattern=$4")
		assert.Equal(t, strings.Count(builder.ToQueryString(), "$"), 4)
		assert.Contains(t, builder.GetArgsValue(), 4321)
	})
}

func TestItCanGenerateSelectQueryPgsql(t *testing.T) {
	builder := builder.NewPgsqlBuilder(context.Background(), nil, "testing_select")
...
func TestItCanGenerateJoinTableQueryPgsql(t *testing.T) {
	builder := builder.NewPgsqlBuilder(context.Background(), nil, "testing_join")
...
func TestItCanGenerateOrderByQueryPgsql(t *testing.T) {
	builder := builder.NewPgsqlBuilder(context.Background(), nil, "testing_order")
